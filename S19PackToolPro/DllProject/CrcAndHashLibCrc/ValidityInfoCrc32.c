...
void InvertUint32(u32 *pDst, u32 *pSrc)
{
    u32 i;
    u32 ulVal = 0;
...
/* TC334 App激活信息中的 AppCrc32 值计算（硬件算法） */
__declspec(dllexport) u32 GetCrc32ForForValidityInfo(void *buffer, u32 len)
{
    u32 i;
    u08 *pbuf = (u08*)buffer, chr;
    u32 crc = 0;
    ...
    for (i = 0; i < len;)
    {
        ...
        {
            chr = pbuf[j];
            crc = CRC_Table[(crc ^ chr) & 0xFF] ^ (crc >> 8);
        }
        pbuf += 4;
        i += 4;
        if (i % (0x3FFF * 4) == 0)
